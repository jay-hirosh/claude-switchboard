...
- **"Now running" in the popover.** See live Claude Code sessions as they happen — project, model, running cost, and elapsed time — right in the compact popover, no need to switch to a terminal to check whether something's still working. Shows up to 3 sessions with an overflow count beyond that.
- **Session-finished notifications.** When a session that's run 10+ minutes goes quiet, a notification tells you it's done, with its final cost and duration. Toggle in Settings → Notifications, on by default.
- **Context-window warnings.** A notification, plus an amber indicator on the session's "Now running" row, when a session's context crosses 80% of its model's window — a heads-up before auto-compaction summarizes it away. Toggle in Settings → Notifications, on by default.
- **Best-account indicator.** When you manage two or more accounts, a "Best available" badge marks whichever one currently has the most rate-limit headroom, so you don't have to compare every account's bars by eye before switching.
- **Pay-as-you-go dollars and forecast.** The PAYG row now shows actual dollars spent against your limit (`$31.20 of $100.00`) and, once there's enough polling history, a projected month-end spend (`→ ~$52 by reset`). PAYG credit alerts also get their own threshold, separate from the shared 5H/7D sliders.
...
- **Model routing advisor.** When your Opus and Sonnet 7-day buckets diverge sharply (one near its limit, the other lightly used), a dismissible hint in the popover suggests switching models — dismissal is scoped to the current 7-day window, so the hint reappears if the gap recurs next week.
- **Limit-hit analytics.** A new Limit hits tab in the expanded report answers "when do I actually run out?" — per-account counts of how often each rate-limit bucket actually got hit, an hour-of-day distribution of when it happened, and the projects active during those windows. Builds forward from when this ships; there's no history to backfill.
- **Smart warm-up suggestions.** Once there's at least 10 days of activity history, the warm-up schedule picker suggests an anchor time based on when you actually tend to start working, with one click to apply it. Purely a suggestion — warm-up stays opt-in and nothing auto-applies.
- **Terminal statusline.** Settings → Terminal statusline installs a Switchboard-provided `switchboard statusline` command into Claude Code's `statusLine` config, showing your 5-hour usage % right in the terminal prompt. Guarded the same way the provider default is: backup, undo record, and a confirmation prompt before overwriting any statusline you already had configured. Only shows a number while Switchboard itself is running — otherwise an honest "not running" placeholder, never a stale one.
- **Model cost-efficiency.** The Models tab now shows cost-per-1K-tokens alongside each model's total cost, so a heavily-used cheap model and a lightly-used expensive one are easy to tell apart at a glance.
- **Period-over-period comparison.** The Trends tab now opens with a comparison strip — cost, tokens, and requests this week vs. last week (or this month vs. last, via a toggle) — so a spike or a slowdown shows up before you even look at the chart.
- **Monthly cost pacing.** The Trends tab now shows a running "$X this month → ~$Y this month" projection, extrapolating from your spend rate so far this calendar month.
- **Anomaly flagging.** The Trends tab chart now marks any day whose cost or tokens ran well above your recent average (2x+) with a small dot — a quiet way to catch a runaway session or accidental loop, not a notification.
- **Sort sessions by duration.** The Sessions tab's Recent/Cost toggle now cycles through Duration too, for finding your longest-running sessions at a glance.
- **CSV export.** The Trends tab has an export button that writes your daily cost/tokens/requests history to a CSV file of your choosing, for slicing in a spreadsheet.
- **7-day limit forecast.** The popover's 7-day bucket now gets the same "→ ~72% by reset" burn-rate projection the 5-hour bucket already had, so you can see whether you're on track to hit your weekly limit before it happens.
