...
Deferred, dependent on the provider model above. Claude Code writes everything required into `~/.claude/projects/*.jsonl` — validated against 154 local transcripts:

| Field | Purpose |
|---|---|
| `sessionId` | `claude --resume <id>` |
| `message.model` | which provider served the session |
| `aiTitle` | recap — already AI-generated by Claude Code |
| `lastPrompt` | secondary recap |
| `cwd`, `gitBranch` | grouping and display |
| `timestamp`, `version` | sorting |

No summarization or inference is required — it is a read-and-render job over files `jsonl_parser` already walks.
