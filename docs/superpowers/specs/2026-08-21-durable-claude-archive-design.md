...
- **Startup backfill** — an initial full sweep of everything in scope (fixed paths + discovered repos), so pre-existing history isn't missed the first time this ships. Runs once, tracked via a `settings` row (`archive_backfill_completed`, same key-value pattern as the existing `migration_completed`/`warmup_consent_granted` rows) so it doesn't re-scan on every launch. Repo discovery itself re-runs on every launch (cheap relative to full backfill, and it's how newly-touched repos start being watched — see non-goals).
